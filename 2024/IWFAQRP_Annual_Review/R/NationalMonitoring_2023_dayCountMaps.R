...
# NOTE:  For a solution the "`breaks` and `labels` are different lengths"
# NOTE:  error message that happens when there is ony one category, see:
# NOTE:  https://community.rstudio.com/t/breaks-and-labels-are-different-lengths/160531/2
...
p1 <- ggplot() +
  ...
  scale_size(breaks = 1:(length(bin_thresholds)-1), labels = \(x)bin_labels[x], name = 'Number of days') +
...
p3 <- ggplot() +
  ...
  scale_size(breaks = 1:(length(bin_thresholds)-1), labels = \(x)bin_labels[x]) +
  # theme_nothing() +
  coord_map()
